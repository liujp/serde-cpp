...
template <class Subtype> class load_inspector_base : public load_inspector {
public:
  ...
  template <class T> bool begin_object_t() {
    return dref().begin_object(type_id_v<T>, type_name_v<T>);
  }

  template <class T> bool list(T &xs) {
    xs.clear();
    auto size = size_t{0};
  ...
  template <class T> bool map(T &xs) {
    xs.clear();
    auto size = size_t{0};
    ...
    for (size_t i = 0; i < size; ++i) {
      ...
      if constexpr (is_pair<decltype(emplace_result)>::value) {
        if (!emplace_result.second) {
          super::emplace_error(error_code::runtime_error,
  ...
  bool tuple(T &xs, std::index_sequence<Is...>) {
    return dref().begin_tuple(sizeof...(Is)) &&
           (load(dref(), std::get<Is>(xs)) && ...) //
           && dref().end_tuple();
  }

  template <class T> bool tuple(T &xs) {
    ...
    if constexpr (std::is_same<setter_result, bool>::value) {
      if (dref().apply(tmp)) {
        if (set(std::move(tmp))) {
          ...
          super::emplace_error(error_code::save_callback_failed);
          return false;
        }
      } else {
        return false;
